...
nonfraud_train_query_rest = f"""\
SELECT * FROM NU
WHERE fraud = 0
AND status = 'PAYOUT_SUCCESSFUL'
AND date(created_at) BETWEEN '2023-01-01' AND '2023-08-31'
"""

nonfraud_train_query_sept = f"""\
SELECT * FROM NU
WHERE fraud = 0
AND status = 'PAYOUT_SUCCESSFUL'
AND date(created_at) BETWEEN '2023-09-01' AND '2023-09-30'
"""
...
nonfraud_train_rest = spark.sql(nonfraud_train_query_rest).toPandas()
nonfraud_train_sept = spark.sql(nonfraud_train_query_sept).toPandas()
...
nonfraud_train_rest = nonfraud_train_rest.dropna(subset=['ga_id', 'session_id'])
nonfraud_train_sept = nonfraud_train_sept.dropna(subset=['ga_id', 'session_id'])
nonfraud_test = nonfraud_test.dropna(subset=['ga_id', 'session_id'])
fraud_train = fraud_train.dropna(subset=['ga_id', 'session_id'])
fraud_test = fraud_test.dropna(subset=['ga_id', 'session_id'])

# COMMAND ----------

fraud_count = len(fraud_train)

nonfraud_train_sept = nonfraud_train_sept.sample(n=fraud_count, random_state=1)
nonfraud_train_rest = nonfraud_train_rest.sample(n=3*fraud_count, random_state=1)

# COMMAND ----------

len(nonfraud_train_sept)

# COMMAND ----------

len(nonfraud_train_rest)

# COMMAND ----------

X_train = pd.concat([nonfraud_train_sept, nonfraud_train_rest, fraud_train])
