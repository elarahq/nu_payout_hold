...
FEATURE_TABLE_NAME = 'data_science_metastore.nu_payout_production_tables.MAIN_PROD_LATEST_GA_ID_KAFKA_NU_DATA'

# COMMAND ----------

max_timestamp_df = spark.sql(f"""
select
...
MAX_TIMESTAMP_IN_TABLE = max_timestamp_df.collect()[0]['max_timestamp'] if max_timestamp_df.collect()[0]['max_timestamp'] is not None else 0
print(f'MAX TIMESTAMP :: {MAX_TIMESTAMP_IN_TABLE}')
...
web_query = f"""\
    ...
    WHERE
        date(date) >= date('{MAX_TIMESTAMP_IN_TABLE}')
        and timestamp > '{MAX_TIMESTAMP_IN_TABLE}'
"""
...
kafka_web_data.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable("data_science_metastore.nu_payout_production_tables.6_HOUR_P1_KAFKA_NU_DATA_WEB")

# COMMAND ----------

kafka_app_data.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable("data_science_metastore.nu_payout_production_tables.6_HOUR_P1_KAFKA_NU_DATA_APP")
...
query = """\
    SELECT * FROM data_science_metastore.nu_payout_production_tables.6_HOUR_P1_KAFKA_NU_DATA_WEB
"""
...
query = """\
    SELECT * FROM data_science_metastore.nu_payout_production_tables.6_HOUR_P1_KAFKA_NU_DATA_APP
"""
...
df_web.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable("data_science_metastore.nu_payout_production_tables.6_HOUR_P2_KAFKA_NU_DATA_WEB")

# COMMAND ----------

df_app.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable("data_science_metastore.nu_payout_production_tables.6_HOUR_P2_KAFKA_NU_DATA_APP")
...
query = """\
    ...
    FROM data_science_metastore.nu_payout_production_tables.6_HOUR_P2_KAFKA_NU_DATA_WEB t
    JOIN (
        SELECT
            session_id,
        ...
        FROM data_science_metastore.nu_payout_production_tables.6_HOUR_P2_KAFKA_NU_DATA_WEB
        GROUP BY session_id
...
query = """\
    ...
    sub.number_of_hits,
    ROW_NUMBER() OVER (PARTITION BY t.session_id ORDER BY t.timestamp) AS hit_number
    FROM data_science_metastore.nu_payout_production_tables.6_HOUR_P2_KAFKA_NU_DATA_APP t
    JOIN (
        SELECT
            session_id,
            MIN(timestamp) AS session_start,
            MAX(timestamp) AS session_end,
            COUNT(*) AS number_of_hits
        FROM data_science_metastore.nu_payout_production_tables.6_HOUR_P2_KAFKA_NU_DATA_APP
        GROUP BY session_id
    ) sub ON t.session_id = sub.session_id;
"""
...
df_web.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable("data_science_metastore.nu_payout_production_tables.6_HOUR_P3_KAFKA_NU_DATA_WEB")

# COMMAND ----------

df_app.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable("data_science_metastore.nu_payout_production_tables.6_HOUR_P3_KAFKA_NU_DATA_APP")
...
p3_web_query = f"""\
    SELECT
        *
    FROM
        data_science_metastore.nu_payout_production_tables.6_HOUR_P3_KAFKA_NU_DATA_WEB
    WHERE
        session_id IN (
            SELECT
                session_id
...
p3_app_query = f"""\
    SELECT
        *
    FROM
        data_science_metastore.nu_payout_production_tables.6_HOUR_P3_KAFKA_NU_DATA_APP
    WHERE
        session_id IN (
            select
                session_id
...
p3_kafka_web_data.write.format("delta").mode("overwrite").option("mergeSchema", "true").saveAsTable("data_science_metastore.nu_payout_production_tables.6_HOUR_P4_KAFKA_NU_DATA_WEB")

# COMMAND ----------

p3_kafka_app_data.write.format("delta").mode("overwrite").option("mergeSchema", "true").saveAsTable("data_science_metastore.nu_payout_production_tables.6_HOUR_P4_KAFKA_NU_DATA_APP")
...
app = spark.sql("select * from data_science_metastore.nu_payout_production_tables.6_HOUR_P4_KAFKA_NU_DATA_APP")
web = spark.sql("select * from data_science_metastore.nu_payout_production_tables.6_HOUR_P4_KAFKA_NU_DATA_WEB")

combined = app.union(web)

# COMMAND ----------

combined.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable("data_science_metastore.nu_payout_production_tables.6_HOUR_FINAL_KAFKA_NU_DATA")
...
query = """\
    WITH base AS (
        ...
        (
            ...
            FROM (
                ...
                FROM data_science_metastore.nu_payout_production_tables.6_HOUR_FINAL_KAFKA_NU_DATA
            ) AS T1
            LEFT JOIN (
                SELECT ga_id, session_id, timestamp, session_start, session_time
                FROM data_science_metastore.nu_payout_production_tables.6_HOUR_FINAL_KAFKA_NU_DATA
    ...
    FROM data_science_metastore.nu_payout_production_tables.6_HOUR_FINAL_KAFKA_NU_DATA NU
...
nu_kafka.write.format("delta").mode("overwrite").option("mergeSchema", "true").saveAsTable("data_science_metastore.nu_payout_production_tables.6_HOUR_FINAL_KAFKA_NU_DATA")
...
WITH interim_table AS (
  ...
  FROM 
    data_science_metastore.nu_payout_production_tables.6_HOUR_FINAL_KAFKA_NU_DATA
)
SELECT 
    ga_id,
    order_id,
...
nu_kafka.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable("data_science_metastore.nu_payout_production_tables.6_HOUR_FINAL_KAFKA_NU_DATA")
...
FROM data_science_metastore.nu_payout_production_tables.6_HOUR_FINAL_KAFKA_NU_DATA AS NU
...
nu_kafka_first_session.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable("data_science_metastore.nu_payout_production_tables.6_HOUR_FINAL_KAFKA_NU_DATA")
...
WITH base AS (
    ...
    FROM data_science_metastore.nu_payout_production_tables.6_HOUR_FINAL_KAFKA_NU_DATA AS T1
    LEFT JOIN
    (
...
FROM
    data_science_metastore.nu_payout_production_tables.6_HOUR_FINAL_KAFKA_NU_DATA AS NU
...
nu_kafka_non_poc.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable("data_science_metastore.nu_payout_production_tables.6_HOUR_FINAL_KAFKA_NU_DATA")
...
FROM
    data_science_metastore.nu_payout_production_tables.6_HOUR_FINAL_KAFKA_NU_DATA NU
...
nu_kafka_last_features.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable("data_science_metastore.nu_payout_production_tables.6_HOUR_FINAL_KAFKA_NU_DATA")

# COMMAND ----------

# nu_kafka_last_features.write.format("delta").mode("append").saveAsTable("data_science_metastore.nu_payout_production_tables.ONE_TIME_FINAL_KAFKA_NU_DATA")
