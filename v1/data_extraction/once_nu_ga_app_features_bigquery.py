...
def ga_app_data(date_1, date_2):

    app_query = f"""\
        select
            CASE WHEN array_contains(`hits-customDimensions`.index, 1) THEN 
            `hits-customDimensions`.value[array_position(`hits-customDimensions`.index, 1) - 1] END as ga_id,
            ...
            `hits-eventInfo`.eventlabel:time_stamp as unix_timestamp,
            from_unixtime((visitStartTime), 'yyyy-MM-dd HH:mm:ss.SSS') as time_stamp,
        ...
        from
            housing_bq_data.ga_sessions_app
        where
            concat_ws('-',fullVisitorId, visitId) IN (
                select distinct(concat_ws('-',fullVisitorId, visitId))
                from housing_bq_data.ga_sessions_app
                where
                (
        ...
        (
            ...
            or (`hits-eventInfo`.eventCategory = 'transaction_success_page' and  `hits-eventInfo`.eventAction = 'refer_save_cta')
            or (`hits-eventInfo`.eventCategory = 'transaction_success_page' and  `hits-eventInfo`.eventAction = 'gift_cta')
            or (`hits-eventInfo`.eventCategory = 'transaction_success_page' and  `hits-eventInfo`.eventAction = 'coupon_copied')
    ...
    ga_app_data = spark.sql(app_query)
    ga_app_data = ga_app_data.withColumn("time_stamp", to_timestamp("time_stamp", "yyyy-MM-dd HH:mm:ss.SSS"))
    ga_app_data = ga_app_data.withColumn("session_start", to_timestamp("session_start", "yyyy-MM-dd HH:mm:ss.SSS"))
    ga_app_data = ga_app_data.withColumn("source", lit("app"))

    ga_app_data_v2 = ga_app_data.select('ga_id', 'session_id', 'date', 'category', 'action', 'time_stamp', 'session_start', 'marketing_name', 'brand', 'region', 'city', 'lat', 'long', 'no_hits', 'screen_views', 'unique_screen_views', 'page_views', 'bounces', 'time_on_screen', 'time_on_site', 'traffic_source', 'traffic_campaign', 'traffic_source_medium', 'hits_referer', 'hit_number', 'source')\
...
# removed hit_number from drop duplicate condition as it was exploding the number of rows.
    return ga_app_data_v2

# COMMAND ----------

def ga_app_data_transformation(ga_app_data_v2):

    ga_app_data_v2.createOrReplaceTempView("ga_app_data_view")
    
    ga_app_session_features = spark.sql("""
    SELECT
    ...
    FROM    
        ...
        FROM
          (
            ...
            FROM
              ga_app_data_view T1) AS ord
            LEFT JOIN (
                        SELECT
                          T1.ga_id, T1.session_id, T1.date, T1.category, T1.action, T1.time_stamp, T1.session_start, T1.marketing_name, T1.brand, T1.region, T1.city, T1.lat, T1.long, T1.no_hits, T1.screen_views, T1.unique_screen_views, T1.page_views, T1.bounces, T1.time_on_screen, T1.time_on_site, T1.traffic_source, T1.traffic_campaign, T1.traffic_source_medium, T1.hits_referer, T1.hit_number, T1.source
                        FROM
                          ga_app_data_view T1) AS ord2
    ...
    # ga_app_session_features = ga_app_session_features.withColumn('user_activity', lit(None).cast('integer')).withColumn("source", lit("app"))
    return ga_app_session_features
...
app_data = ga_app_data(date_1, date_2)

# COMMAND ----------

app_data.write.format("delta").mode("overwrite").option("mergeSchema", "true").saveAsTable("feature_store" + "." + "NU_GA_APP_INTERMEDIATE_TABLE")
del(app_data)

# COMMAND ----------

app_data = spark.sql("""SELECT * FROM feature_store.NU_GA_APP_INTERMEDIATE_TABLE""")
app_session_features = ga_app_data_transformation(app_data)

# app_data = app_data.select(
#                       col('date').alias('date'),
...
#                       col('unqiue_screen_views').alias('unique_screen_views'),
...
app_session_features.write.format("delta").mode("overwrite").option("mergeSchema", "true").saveAsTable("feature_store" + "." + "NU_GA_FEATURE_STORE_more_features")
del(app_session_features)
# del(app_session_features)
