...
# MAGIC %sql
# MAGIC SELECT * FROM feature_store.kafka_nu_data WHERE ga_id = '2033514486'
    ...
    WITH base AS (
        ...
        (
            ...
            LEFT JOIN (
                SELECT ga_id, session_id, timestamp, session_start, session_time
                FROM feature_store.KAFKA_NU_DATA
            ) AS T2 ON T2.ga_id = T1.ga_id AND T2.session_start between T1.session_start - INTERVAL '90' DAY AND T1.session_start
        ...
        (
            ...
            GROUP BY T1.ga_id, T1.session_id, T1.timestamp, T1.session_start, T1.session_time
        ) AS Z2 ON Z2.ga_id = Z1.ga_id AND Z2.session_start between Z1.session_start - INTERVAL '90' DAY AND Z1.session_start
...
WITH base AS (
    ...
    ) AS T2 ON T2.ga_id = T1.ga_id AND T2.timestamp between T1.timestamp - INTERVAL '90' DAY AND T1.timestamp
    GROUP BY 1,2,3,4,5
)
